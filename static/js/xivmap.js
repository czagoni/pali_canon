...
function xivmap(config) {
	...
	function makeRectangle(element, ratio, originalElement) {
		...
		var context_rect = position(o.context);

		if (!rectangle.width || !rectangle.height) return '';
		var style = 'style="' +
			'position: absolute; ' +
			'top: ' + r((rectangle.top - context_rect.top) * ratio) + 'px; ' +
			'left: ' + r((rectangle.left - context_rect.left) * ratio) + 'px; ' +
